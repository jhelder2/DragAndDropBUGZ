(()=>{
    ...
    const piecesBoard = document.querySelector(".puzzle-pieces"),
          ...
          puzzleImage = document.querySelectorAll(".puzzle-image")
...
//RESET PUZZLE PIECES [~_~]

  // mAyBe gET CHiLd DaTA AnD APpEnDcHilD THAT!!?!?!?>!??

//SET DATA
          dropZones.forEach(zones => {
              zones.addEventListener(" ", function(e){
                  e.dataTransfer.setData("text/plain", this.id);
//GET DATA
          puzzleImage.forEach(piece => {

              piece.addEventListener(" ", function(e) {

                  let draggedImg = e.dataTransfer.getData("text/plain");

                  e.target.appendChild(document.querySelector(`#${draggedImg}`))
              });
            });
